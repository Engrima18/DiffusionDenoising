...
def extract_loader_features(loader, inception, device, n_samples: int, batch_size: int):
    ...
    for i, (img, _) in tqdm(
        enumerate(loader), desc="extracting ref features for FID: ", leave=False
    ):
        img = img.to(device)
...
def extract_model_features(
    ...
    for _ in tqdm(
        range(num_iters),
        desc="extracting generated data features for FID: ",
        leave=False,
    ):
        ...
        if batch.shape[2] != 299 or batch.shape[3] != 299:
            batch = torch.nn.functional.interpolate(
                batch, size=(299, 299), mode="bicubic"
            )
...
def fid_generator(
    generator, ref_mean, ref_cov, inception, device, num_samples, batch_size=64
):

    gen_feat = extract_model_features(
        generator, inception, device, num_samples=num_samples, batch_size=batch_size
    )
...
def get_fid_between_datasets(
    ...
    feat_a = extract_loader_features(
        loader_a, inception, device, num_samples, batch_size
    )[:num_samples]
    feat_b = extract_loader_features(
        loader_b, inception, device, num_samples, batch_size
    )[:num_samples]
...
def get_fid_fn(dataset, device, batch_size: int, num_samples: int = 50_000):
    ...
    ref_feat = extract_loader_features(
        loader, inception, device, num_samples, batch_size
    )[:num_samples]
