    ...
    def __init__(self, folder_images, image_size, power, from_uv):
        ...
        if "forecast" in self.path_images:
            ...
            self.file_list = sorted(
                os.listdir(os.path.join(folder_images, self.DIRTY_NOISY_WO_PROCESSING))
            )

    def _set_folders(self, folder_images):
        self.dirty_noisy_wo_processing_folder = os.path.join(
            folder_images, self.DIRTY_NOISY_WO_PROCESSING
        )
        self.true_wo_processing_folder = os.path.join(
            folder_images, self.TRUE_WO_PROCESSING
        )
    ...
    def __getitem__(self, index: int):
        if "forecast" in self.path_images:
            ...
            def open_dirty_noisy(file_name):
                dirty_noisy_file = os.path.join(
                    self.dirty_noisy_wo_processing_folder, file_name
                )
        ...
        sky_model = torch.from_numpy(true.astype(np.float32)).float()

        dirty_noisy = (dirty_noisy - 0.5) / 0.5
        dirty_noisy = torch.from_numpy(dirty_noisy.astype(np.float32)).float()

        data["true"] = torch.nn.functional.interpolate(
            sky_model.unsqueeze(0), mode="bicubic", size=self.im_size
        )[0]

        data["dirty_noisy"] = torch.nn.functional.interpolate(
            dirty_noisy.unsqueeze(0), mode="bicubic", size=self.im_size
        )[0]
...
class MakeDataLoader:
    ...
    def __init__(
        ...
        self.dataset = SkaDataset(
            folder_images, image_size, power=power, from_uv=from_uv
        )
        if not augmented:
        ...
        if real_data:
            ...
            valid_idx, test_idx = train_test_split(
                test_idx, test_size=0.5, random_state=random_state + 1
            )
        ...
        if not use_zeros:
            dataset_zero = SkaDataset(
                f"{folder_images}/zero", image_size, power=power, from_uv=from_uv
            )
    ...
    def get_data_loader_full(
        self, batch_size: int = 64, shuffle: bool = True, **kwargs
    ) -> DataLoader:
        return DataLoader(
            self.dataset,
    ...
    def get_data_loader_test(
        self, batch_size: int = 64, shuffle: bool = False, **kwargs
    ) -> DataLoader:
        return DataLoader(
            self.dataset_test,
            batch_size=batch_size,
